module.exports = function(RED) {
  ...
  function GoogleCalendarInputNode(config) {
    ...
    this.calendar = config.calendar || 'primary';
    if (!config.offsetType || config.offsetType === 'at') {
      this.offset = 0;
    } else {
      var plusOrMinus = config.offsetType === 'before' ? 1 : -1;
      var multiplier = {
        seconds: 1000,
        minutes: 60*1000,
        hours: 60*60*1000,
      ...
      this.offset = plusOrMinus * config.offset * multiplier;
    }

    var setNextTimeout;
    var eventsBetween;
    if (!config.offsetFrom || config.offsetFrom === 'start') {
  ...
  function setNextStartingTimeout(node, cal, after, cb) {
    ...
    after = new Date(after.getTime()+node.offset);
  ...
  function GoogleCalendarQueryNode(config) {
    RED.nodes.createNode(this,config);
    this.google = RED.nodes.getNode(config.google);
    this.calendar = config.calendar || 'primary';
  ...
  function calendarByNameOrId(node, nameOrId) {
    return node.calendars.hasOwnProperty(nameOrId)?node.calendars[nameOrId]:calendarByName(node, nameOrId);
  }
  ...
  function eventsStartingBetween(node, cal, msg, start, end, results, cb) {
    ...
    start = new Date(start.getTime()+node.offset);
    end = new Date(end.getTime()+node.offset);
    var request = {
      url: 'https://www.googleapis.com/calendar/v3/calendars/'+cal.id+'/events'
    };
    request.qs = {
  ...
  function GoogleCalendarOutNode(config) {
    RED.nodes.createNode(this,config);
    this.google = RED.nodes.getNode(config.google);
    this.calendar = config.calendar || 'primary';
